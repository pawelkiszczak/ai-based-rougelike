...
func process_run(run_result: Dictionary, codex_ids: Array[String] = []) -> Dictionary:
	...
	var environment_id: String = str(run_result.get("environment_id", ""))
	...
	var saved: Dictionary = save_system.save(save_data)
	if not saved.ok:
...
func _normalise(input: Dictionary) -> Dictionary:
	var output: Dictionary = DEFAULT_STATS.duplicate(true)
